...
# ====== Splash / Menú inicial estilo agenda 80s ======

# (1) Patrones de tramado para “sombras” (2x2 repetidas)
PAT_SOLID = 0b1111
PAT_MED   = 0b0101  # tablero
PAT_LIGHT = 0b0001  # muy suave

def fill_rect_pat(x, y, w, h, pat=PAT_MED):
    # Dibuja un rectángulo "relleno" con patrón 2x2 en 1 bit
    for yy in range(y, y+h):
        for xx in range(x, x+w):
            # índice del subpixel 2x2: (x&1) + 2*(y&1)
            idx = (xx & 1) + ((yy & 1) << 1)
            if (pat >> idx) & 1:
                d.rectangle(xx, yy, 1, 1)

def draw_cartela(x, y, w, h, text_):
    # Marco con "dentado" ligero: borde y relleno tramado
    rect_frame(x, y, w, h, 1)
    fill_rect_pat(x+1, y+1, w-2, h-2, PAT_LIGHT)
    use_font_small()
    d.text(text_, x + (w - len(text_)*6)//2, y + (h-6)//2, w, 1)
    use_font_normal()

def month_abbr_es(m):  # 1..12
    return ["ENE","FEB","MAR","ABR","MAY","JUN","JUL","AGO","SEP","OCT","NOV","DIC"][m-1]

def draw_shelf_area():
    # Estantería: línea superior
    d.line(2, 10, W-2, 10)
    # Estante (banda bajo las cartelas)
    d.line(2, 30, W-2, 30)

def draw_books_right():
    # Libros / botellas a la derecha (formas simples)
    x0 = W-40; base = 30
    # libro alto
    d.rectangle(x0, base-16, 6, 16)
    # botella redondeada
    d.rectangle(x0+8, base-12, 6, 12)
    d.circle(x0+11, base-12, 3)
    # libro tumbado (lomo)
    d.rectangle(x0+16, base-8, 10, 8)
    # archivadores
    d.rectangle(x0+28, base-14, 4, 14)
    d.rectangle(x0+34, base-18, 4, 18)

def draw_desk_area():
    # Superficie escritorio + repisa con tramado
    y = 36
    d.line(2, y, W-2, y)
    fill_rect_pat(2, y+1, W-4, 10, PAT_LIGHT)
    # Borde frontal
    d.line(2, y+12, W-2, y+12)

def draw_device(x, y, w, h, label, selected=False):
    # “Dispositivo” oblicuo: base, tapa y luz
    rect_frame(x, y, w, h, 1)
    # tapa (zona superior)
    fill_rect_pat(x+2, y+2, w-4, h-8, PAT_MED)
    # “cable/lámpara” simplificada
    d.line(x+w-6, y+h-6, x+w-10, y+h-2)
    # etiqueta bajo el dispositivo
    use_font_small()
    if selected:
        rect_frame(x-2, y-2, w+4, h+4, 1)
    d.text(label, x + (w - len(label)*6)//2, y + h + 2, w, 1)
    use_font_normal()

def draw_time_boxes():
    tm = time.localtime()
    mes = month_abbr_es(tm[1])
    hh = tm[3]; mm = tm[4]
    hhmm = "{}:{:02d}".format(hh, mm)
    # Cartelas a la izquierda
    draw_cartela(4, 14, 36, 14, mes)
    draw_cartela(44, 14, 36, 14, hhmm)

def splash_menu():
    # Gestión de 6 accesos: 1..6
    # Ajusta nombres/funciones según tengas disponibles
    entries = [
        ("1 Ctc", lambda: contacts_app()),
        ("2 Memo", lambda: memos_app()),
        ("3 Reloj", lambda: clock_view()),
        ("4 Cal", lambda: calendar_view()),
        ("5 Calc", lambda: calculator_app()),
        ("6 Mundo", lambda: world_clock_view()),
        # Si quieres 7/8:
        # ("7 Snake", lambda: snake_game()),
        # ("8 Conf",  lambda: settings_app()),
    ]
    sel = 0
    last_time_draw = 0

    # Geometría de los 6 “dispositivos” sobre el escritorio
    base_y = 40
    w_dev, h_dev = 34, 14
    gap = 3
    # 3 arriba, 3 abajo, centrados
    total_w = 3*w_dev + 2*gap
    start_x = (W - total_w)//2
    slots = []
    for row in range(2):
        for col in range(3):
            x = start_x + col*(w_dev + gap)
            y = base_y + row*(h_dev + 12)  # segunda fila más abajo
            slots.append((x, y))

    while True:
        # Redibuja ~5 veces por segundo o cuando hay tecla
        now = time.ticks_ms()
        if time.ticks_diff(now, last_time_draw) > 200:
            cls()
            header("Agenda 80s")
            # Estantería y libros
